...
public class Packet {
	...
	private static final Segments[] BODY_SEGMENTS = {Segments.bMsq, Segments.wBodyCrc16};
	...
	public String toString() {
		return "Packet{" + "clientAddress=" + clientAddress + ", clientPort=" + clientPort + ", BMagic=" + getBMagic() +
				", BSrc=" + getBSrc() + ", BPktId=" + getBPktId() + ", WLen=" + getWLen() + ", WHeaderCrc16=" +
				getWHeaderCrc16() + ", bMsq=" + getbMsq() + ", WBodyCrc16=" + getWBodyCrc16() + '}';
	}
			...
			for (Segments segment : SEGMENTS) {
				...
				if (segment != Segments.bMsq && data.get(segment).length != segment.getLength())
					throw new IllegalStateException(
							"Invalid data! " + segment + " size = " + data.get(segment).length + ", but should be " +
									segment.getLength());
			}
